let CoverflowStyle = {
	...
	insertSheet(){
		...
		minifiedStyle = this._sheet["2d"].replace(ruleFormater, "");
		base64Style = btoa(minifiedStyle);
		d2Style.href = `data:text/css;base64,${base64Style}`;

		Logger.debug("[FlickrCoverflow] - insertSheet - flickrCoverflow-style-2D:", minifiedStyle);
		...
		if(this._3d){
			...
			minifiedStyle = this._sheet["3d"].replace(ruleFormater, "");
			base64Style = btoa(minifiedStyle);
			d3Style.href = `data:text/css;base64,${base64Style}`;

			Logger.debug("[FlickrCoverflow] - insertSheet - flickrCoverflow-style-3D:", minifiedStyle);
