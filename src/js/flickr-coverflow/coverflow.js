...
class Coverflow {
  ...
  constructor ({apiKey, user, container, size = 'small', '3d': d3 = false } = {
    ...
    if (!container || !(container instanceof HTMLElement)) {
      throw `[flickr-coverflow/Coverflow] - constructor - parameter container is required and must be an HTMLElement. Actual: ${container}`
    }

    if (!container.id || document.querySelectorAll(`#${container.id}`).length > 1) {
      throw `[flickr-coverflow/Coverflow] - constructor - parameter container must have a unique id. Actual: ${container.id}`
    }

    if (container.classList.contains(SheetList.cssClass)) {
      Logger.warn(`[flickr-coverflow/Coverflow] - constructor - #${container.id} is already a coverflow`)
      return
    }
  ...
  _validateStringArg (name, value, possibleValues) {
    if (! possibleValues) {
      if (typeof value !== 'string' || ! value.trim()) {
        throw `[flickr-coverflow/Coverflow] - _validateStringArg - parameter ${name} is required and must be a non-empty string. Actual: ${value}`
      }
    } else {
      if (possibleValues.indexOf(value) < 0) {
        throw `[flickr-coverflow/Coverflow] - _validateStringArg - parameter ${name} must be one of ${possibleValues}. Actual: ${value}`
      }
    }
  }
