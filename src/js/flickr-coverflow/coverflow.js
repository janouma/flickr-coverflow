...
class Coverflow {
  ...
  static _UNIVERSAL_CLICK = Coverflow._touchEnabled ? 'tap' : 'click'
  ...
  constructor({apiKey, user, container, size = 'medium', '3d': d3 = false } = {
    ...
    if (!container || !(container instanceof HTMLElement)) {
      throw `${Coverflow._CLASS_ID} - constructor - parameter container is required and must be an HTMLElement. Actual: ${container}`
    }

    if (!container.id || document.querySelectorAll(`#${container.id}`).length > 1) {
      throw `${Coverflow._CLASS_ID} - constructor - parameter container must have a unique id. Actual: ${container.id}`
    }
  ...
  _validateStringArg(name, value, possibleValues) {
    if (!possibleValues) {
      if (typeof value !== 'string' || !value.trim()) {
        throw `${Coverflow._CLASS_ID} - _validateStringArg - parameter ${name} is required and must be a non-empty string. Actual: ${value}`
      }
    } else {
      if (possibleValues.indexOf(value) < 0) {
        throw `${Coverflow._CLASS_ID} - _validateStringArg - parameter ${name} must be one of ${possibleValues}. Actual: ${value}`
      }
    }
  }
  ...
  _attachZoomEvent() {
    ...
    this._container.addEventListener(U_CLICK, event => {
      ...
      if (frame.classList.contains(FRAME_CSS_CLS)
        ...
        Logger.debug(`${Coverflow._CLASS_ID} - _attachZoomEvent - "${U_CLICK}" triggered, about to trigger "zoom"`)
  ...
  _attachTouchEvents() {
    ...
    container.addEventListener('touchstart', event => {
      ...
      Logger.debug(`${Coverflow._CLASS_ID} - _attachTouchEvents - this._startX:`, this._startX)
    }, false)

    container.addEventListener('touchmove', ::this._onTouchMove, false)
    container.addEventListener('touchend', () => this._deltaX = 0, false)
  }
